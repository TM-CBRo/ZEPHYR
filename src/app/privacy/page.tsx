    ...
    <main className="min-h-screen bg-[#020617] text-slate-200 font-sans selection:bg-emerald-500/30 overflow-x-hidden pb-32">
      ...
      <nav className="relative z-50 border-b border-slate-800/40 bg-[#020617]/80 backdrop-blur-xl">
        <div className="max-w-5xl mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <Link href="/" className="text-sm font-medium text-slate-400 hover:text-emerald-400 transition-colors flex items-center gap-2">
      ...
      <section className="relative z-10 pt-32 pb-20 px-6 max-w-4xl mx-auto text-center">
        <motion.div initial="hidden" animate="visible" variants={fadeUp}>
          ...
          <h1 className="text-4xl md:text-7xl font-extrabold tracking-tighter mb-6 text-white">
            Privacy by <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-cyan-400">Design.</span>
          </h1>
          <p className="text-lg md:text-xl text-slate-400 font-light leading-relaxed max-w-2xl mx-auto">
            Securitatea cibernetică reală nu ar trebui să coste intimitatea utilizatorului. Am construit arhitectura în jurul principiului "Zero-Knowledge".
          </p>
        </motion.div>
      </section>

      <motion.section variants={staggerContainer} initial="hidden" animate="visible" className="relative z-10 px-6 max-w-4xl mx-auto space-y-12">
        
        {/* Card 1: JSON Data Minimization (Live Stream) */}
        <motion.div variants={fadeUp} className="bg-slate-900/40 border border-slate-800/60 rounded-3xl p-8 md:p-12 backdrop-blur-sm overflow-hidden flex flex-col md:flex-row items-center gap-10">
          <div className="flex-1">
            <h3 className="text-3xl font-bold text-slate-100 mb-4 tracking-tight">1. Minimizarea Datelor Absolute</h3>
            <p className="text-slate-400 leading-relaxed mb-4">
              Conform <strong>Articolului 5(1)(c) din GDPR</strong>, procesăm doar datele strict necesare. Motorul nostru este „orb” intenționat. Extragem doar headerele și izolăm linkurile. Corpul emailului nu ajunge niciodată în server.
            </p>
          </div>
          ...
          <div className="w-full md:w-auto bg-[#020617] border border-slate-800 p-5 rounded-2xl font-mono text-[11px] md:text-xs text-left shadow-2xl flex-shrink-0">
             ...
             <div className="pl-4 space-y-1">
                ...
                <div className="flex flex-wrap">
                  <span className="text-emerald-400 mr-2">"extracted_links":</span> 
                ...
                <div className="bg-rose-500/10 border border-rose-500/30 p-2 rounded mt-2">
                  <span className="text-rose-500">"body_content"</span>: <span className="text-rose-400 font-bold px-1 rounded animate-pulse">{"<REDACTED BY ZEPHYR>"}</span>
                </div>
             </div>
             <div className="text-slate-300">{`}`}</div>
          </div>
        ...
        <motion.div variants={fadeUp} className="bg-[#040914] border border-slate-800/60 rounded-3xl p-8 md:p-12 backdrop-blur-sm">
          <div className="flex flex-col md:flex-row gap-10 items-center">
            <div className="flex-1">
              <h3 className="text-3xl font-bold text-slate-100 mb-4 tracking-tight">2. Dreptul de a fi uitat (Automat)</h3>
              <p className="text-slate-400 leading-relaxed mb-6">
                <strong>Articolul 17 GDPR.</strong> Baza noastră de date (Firestore) folosește o politică Time-To-Live (TTL) care distruge automat orice urmă a logurilor de securitate la exact 30 de zile.
              </p>
            </div>

            <div className="w-full md:w-1/2 bg-[#020617] border border-slate-800 p-6 rounded-2xl relative overflow-hidden">
              {isPurging && <div className="absolute inset-0 bg-rose-500/20 z-0"></div>}
              
              <div className="relative z-10 flex justify-between items-end mb-4">
                 <span className="text-xs uppercase text-slate-500 font-bold tracking-wider">Ciclu Retenție Date</span>
                 <span className={`text-xl font-mono font-bold ${isPurging ? 'text-rose-500' : 'text-emerald-400'}`}>
              ...
              <div className="w-full h-4 bg-slate-900 rounded-full overflow-hidden shadow-inner border border-slate-800 relative">
                 <motion.div 
                   animate={{ width: isPurging ? '100%' : `${(days / 30) * 100}%` }}
              ...
              <p className="text-[10px] text-slate-500 text-center mt-4 font-mono uppercase tracking-widest">
                {isPurging ? 'Trigger TTL executat. Date șterse.' : 'Acumulare metadate statistice...'}
              </p>
            </div>
          </div>
        </motion.div>
